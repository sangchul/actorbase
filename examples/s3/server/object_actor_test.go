...
func TestObjectActor_Put(t *testing.T) {
	...
	resp, walEntry, err := a.Receive(nil, s3common.ObjectRequest{
...
func TestObjectActor_Get(t *testing.T) {
	a := newObject()
	a.Receive(nil, s3common.ObjectRequest{
		Op: "put", Bucket: "photos", Key: "2024/cat.jpg",
		Size: 2048, ETag: "def456", StorageClass: "STANDARD_IA",
	})

	t.Run("existing", func(t *testing.T) {
		resp, walEntry, err := a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "2024/cat.jpg"})
		require.NoError(t, err)
		require.True(t, resp.Found)
	...
	t.Run("not found", func(t *testing.T) {
		resp, _, err := a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "2024/dog.jpg"})
		require.NoError(t, err)
...
func TestObjectActor_Delete(t *testing.T) {
	a := newObject()
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "2024/cat.jpg", Size: 1024, ETag: "abc"})

	_, walEntry, err := a.Receive(nil, s3common.ObjectRequest{Op: "delete", Bucket: "photos", Key: "2024/cat.jpg"})
	require.NoError(t, err)
	require.NotNil(t, walEntry, "delete should produce a WAL entry")

	resp, _, _ := a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "2024/cat.jpg"})
	require.False(t, resp.Found, "object should not be found after delete")
}

func TestObjectActor_UnknownOp(t *testing.T) {
	a := newObject()
	_, _, err := a.Receive(nil, s3common.ObjectRequest{Op: "copy"})
	require.Error(t, err)
}
...
func TestObjectActor_WALReplay(t *testing.T) {
	...
	a := newObject()
	_, walCat, _ := a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 100, ETag: "e1"})
	_, walDog, _ := a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "dog.jpg", Size: 200, ETag: "e2"})
	_, walBird, _ := a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "bird.jpg", Size: 300, ETag: "e3"})
	_, walDeleteDog, _ := a.Receive(nil, s3common.ObjectRequest{Op: "delete", Bucket: "photos", Key: "dog.jpg"})
	...
	for key, shouldExist := range map[string]bool{"cat.jpg": true, "dog.jpg": false, "bird.jpg": true} {
		resp, _, _ := restored.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: key})
...
func TestObjectActor_SnapshotRestore(t *testing.T) {
	...
	original := newObject()
	original.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 100, ETag: "e1", StorageClass: "STANDARD"})
	original.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "videos", Key: "movie.mp4", Size: 99999, ETag: "e2", StorageClass: "GLACIER"})
	...
	for _, tc := range []struct{ bucket, key, storageClass string }{
		...
		resp, _, _ := restored.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: tc.bucket, Key: tc.key})
...
func TestObjectActor_Split(t *testing.T) {
	...
	a := newObject()
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "alpha", Key: "obj1", Size: 1, ETag: "e1"})
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "alpha", Key: "obj2", Size: 2, ETag: "e2"})
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "beta", Key: "obj1", Size: 3, ETag: "e3"})
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "gamma", Key: "obj1", Size: 4, ETag: "e4"})
	...
	for _, tc := range []struct{ bucket, key string }{{"beta", "obj1"}, {"gamma", "obj1"}} {
		resp, _, _ := upper.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: tc.bucket, Key: tc.key})
	...
	for _, tc := range []struct{ bucket, key string }{{"alpha", "obj1"}, {"alpha", "obj2"}} {
		resp, _, _ := a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: tc.bucket, Key: tc.key})
...
func TestObjectActor_KeyCount(t *testing.T) {
	...
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 1, ETag: "e1"})
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "dog.jpg", Size: 2, ETag: "e2"})
	require.EqualValues(t, 2, a.KeyCount())

	a.Receive(nil, s3common.ObjectRequest{Op: "delete", Bucket: "photos", Key: "cat.jpg"})
...
func TestObjectActor_WALEntryFormat(t *testing.T) {
	...
	a := newObject()
	_, walEntry, _ := a.Receive(nil, s3common.ObjectRequest{
...
func TestObjectActor_LastModifiedPreservedOnReplay(t *testing.T) {
	...
	a := newObject()
	_, walEntry, _ := a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 1, ETag: "e1"})
...
func TestObjectActor_SplitHint(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "cat.jpg"})
	}
	for i := 0; i < 2; i++ {
		a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "dog.jpg"})
	}
	a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "bird.jpg"})
	...
	t.Run("split at hint separates hotspot", func(t *testing.T) {
		...
		a2 := newObject()
		a2.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "bird.jpg", Size: 1, ETag: "e1"})
		a2.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 2, ETag: "e2"})
		a2.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "dog.jpg", Size: 3, ETag: "e3"})
		for i := 0; i < 10; i++ {
			a2.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "cat.jpg"})
		}
		...
		for _, key := range []string{"cat.jpg", "dog.jpg"} {
			resp, _, _ := upper.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: key})
			require.True(t, resp.Found, "photos/%s should be in upper partition", key)
		}
		// "bird.jpg" should remain in the lower partition
		resp, _, _ := a2.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "bird.jpg"})
...
func TestObjectActor_PutOverwrite(t *testing.T) {
	...
	a := newObject()
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 100, ETag: "v1"})
	a.Receive(nil, s3common.ObjectRequest{Op: "put", Bucket: "photos", Key: "cat.jpg", Size: 200, ETag: "v2"})

	resp, _, _ := a.Receive(nil, s3common.ObjectRequest{Op: "get", Bucket: "photos", Key: "cat.jpg"})

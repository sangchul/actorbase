...
| **자동 rebalancing** | ThresholdPolicy / RelativePolicy (런타임 교체 가능) | 자동 (크기 임계값 + balancer) | 자동 resharding | 자동 vnodes | 해당 없음 | 자동 (Region split) | 자동 |
| **Master/Coordinator HA** | etcd election 기반 Active/Standby | Active/Standby (ZooKeeper election) | 각 shard 자체 election | 없음 (peer-to-peer) | Raft 리더 선출 | PD leader election | 완전 관리형 |
...
| **쿼리** | Key lookup + **Range scan** (multi-partition fan-out) | Row key range scan, column filter | Key lookup, scan | CQL (range, filter) | Key/prefix lookup | Key/range lookup | 제한적 (GSI) |
...
#### 4. Master HA: Active/Standby vs etcd election

HBase HMaster는 ZooKeeper leader election으로 Active/Standby HA를 지원한다.
Standby Master는 HMaster 장애 시 수 초 내 승격된다.

actorbase PM은 etcd `concurrency.Campaign`으로 동일한 Active/Standby 방식을 구현한다.
여러 PM 인스턴스를 띄워도 리더 한 대만 gRPC 서버를 열고, 리더 종료 시 etcd lease가
revoke되면 standby 중 하나가 자동으로 리더에 선출된다. 모든 상태(라우팅 테이블, policy)는
etcd에 있으므로 리더 전환 후 즉시 복원된다.

#### 5. 자동 rebalancing: 내장 vs 정책 주입

HBase는 Region 크기가 임계값(기본 10GB)을 초과하면 자동 split하고,
Load Balancer가 RegionServer 간 Region 수를 균등하게 유지한다.

actorbase는 `ThresholdPolicy`(RPS/key count 절대 임계값)와 `RelativePolicy`(클러스터 평균 대비 배수)를 구현한다.
PM이 `check_interval`마다 모든 PS에 GetStats를 병렬 호출하여 stats를 수집하고, policy가 split/migrate 여부를 결정한다.
`abctl policy apply policy.yaml`로 런타임 교체 가능하며, AutoPolicy 활성 중에는 수동 명령을 거부한다.

### 요약: actorbase가 HBase에서 빌린 것과 단순화한 것

| 항목 | HBase | actorbase |
|---|---|---|
| 복제 없이 shared storage 의존 | HDFS | S3 / NFS / Redis Streams |
...
| Master HA | Active/Standby (ZooKeeper) | Active/Standby (etcd election, 구현됨) |
| 자동 rebalancing | 내장 | ThresholdPolicy / RelativePolicy (구현됨) |
| 서버 측 커스텀 로직 | Coprocessor (제한적) | Actor 인터페이스 (자유도 높음) |
| Split key 결정 | HMaster 또는 midpoint | Actor (`SplitHinter`) 또는 midpoint |
| Merge / Compaction | Minor/Major compaction, Region merge | 미지원 (Partition 수 단조 증가) |
| Range scan | 네이티브 지원 | **SDK fan-out 방식으로 지원** (Actor.Receive → list op, SDK가 파티션 병렬 조회) |

### 저장 용량 모델

actorbase는 메모리 기반 시스템이 아니다. Partition은 메모리에 올라가 있을 때만 처리되며, 유휴 Partition은 checkpoint 저장 후 메모리에서 evict된다. 따라서:

- 총 저장 용량 = CheckpointStore 용량 (HDFS / S3 / NFS)
- 총 메모리 = 동시 활성 Partition 수 × Partition당 메모리
- 데이터가 커지면 Partition을 split하여 더 작은 단위로 나누고, 유휴 Partition은 evict

메모리 총합보다 훨씬 큰 데이터를 저장할 수 있으며, 이 점에서 HBase와 동일하다.

### Redis Streams WAL + HDFS Checkpoint 배포 구성

```
[PS-1] Actor goroutine
    │ AppendBatch
    ▼
[Redis Streams]  ← 파티션별 stream key (Redis Cluster replication으로 내구성)
    │ replay on failover
    ▼
[PS-2] 복원된 Actor
...
[PS-1] Checkpoint
    ...
    ▼
[HDFS] /actorbase/checkpoint/{partitionID}  (블록 3중 복제로 내구성)
    │ CheckpointStore.Load on migration / failover
    ▼
[PS-2] 복원된 Actor
```

이 조합은 HBase(HDFS WAL + HDFS HFile)와 동일한 내구성 보장을 제공하면서, HDFS에 대한 의존을 WAL 계층에서 Redis Streams로 분리한다.

### Merge / Compaction 부재의 영향

HBase는 Region merge와 Compaction(minor/major)으로 파일 수와 Region 수를 관리한다. actorbase에는 Partition merge가 없다. 트래픽이 줄어도 split된 Partition이 자동으로 합쳐지지 않으므로 Partition 수는 단조 증가하는 경향이 있다.

**영향**:
- 장기간 운영 시 소규모 Partition이 누적될 수 있다
- 읽기 성능 저하는 없음 (각 Partition이 독립적으로 checkpoint에서 로드)
- WAL 파일은 checkpoint 이후 trim되므로 WAL 누적은 발생하지 않음

**완화**: split threshold(RPS, key count)를 충분히 높게 설정하면 불필요한 split 억제 가능.
...
### B. PM HA — etcd election 기반 Active/Standby

**현황**: PM은 etcd `concurrency.Campaign`으로 리더 선출을 구현한다. 여러 PM 인스턴스를 동시에 기동하면 하나만 리더가 되어 gRPC 서버를 열고, 나머지는 standby로 대기한다.

```
TiKV: PD leader election으로 HA
etcd: Raft로 HA
actorbase PM: etcd election 기반 Active/Standby (구현됨)
```

**리더 전환 흐름**:
1. 리더 PM 종료 → `sess.Close()`로 etcd lease revoke
2. standby PM 중 하나가 `Campaign()`에서 리더로 선출
3. 새 리더가 etcd에서 라우팅 테이블·policy 복원 후 gRPC 서버 오픈

**가용성 간격**: 리더 종료 후 etcd TTL(~10초) 만료까지 새 리더가 gRPC를 열지 않는다.
이 기간 동안 SDK는 캐시된 라우팅으로 PS에 직접 요청 가능하나, split/migrate/WatchRouting은 불가하다.

**SDK HA 연동**: `sdk.Config.EtcdEndpoints`를 지정하면 PM 연결이 끊길 때 etcd에서 새 리더 주소를 재발견해 자동 재연결한다.

---

### C. 자동 rebalancing

**현황**: ThresholdPolicy와 RelativePolicy가 구현됐다. PM이 check_interval마다 모든 PS에 GetStats RPC를 호출하여 stats를 수집하고, policy가 split/migrate 여부를 결정한다.

```
TiKV:  Region이 크기 임계값 초과 시 자동 split + 자동 이동
actorbase: ThresholdPolicy (RPS/key count 절대 임계값), RelativePolicy (클러스터 평균 RPS 대비 배수)
           abctl policy apply policy.yaml 로 런타임 교체 가능
           AutoPolicy 활성 중에는 수동 split/migrate 명령 거부 (PermissionDenied)
```

**한계**: Partition merge/compaction 없음. split된 Partition은 자동으로 합쳐지지 않는다.
...
| 고가용성 (PM SPOF 허용 불가) | **지원** (etcd election 기반 Active/Standby) |
| 자동 rebalancing 필요 | **지원** (ThresholdPolicy / RelativePolicy) |
...
| 한계 | 심각도 | 향후 과제 |
|---|---|---|
| PM leader 전환 중 가용성 간격 (~10초) | 중간 | etcd TTL 단축 또는 standby PM 선배치로 완화 |
| Partition merge/compaction 없음 | 중간 | 향후 merge 정책 검토 |

...
| 기타 서버 내부 오류 | `INTERNAL` | `fmt.Errorf("internal server error: %s", msg)` 반환 |
...
> **`INTERNAL` 코드 역변환 주의**: `toGRPCStatus`에서 `ErrActorPanicked`와 기타 내부 오류 모두 `INTERNAL`로 인코딩된다.
> `fromGRPCStatus`는 메시지 문자열 비교로 `ErrActorPanicked`를 구분하며, 메시지가 다르면 `fmt.Errorf("internal server error: %s", msg)`를 반환한다.
> 이렇게 하지 않으면 migration 중 "partition already on node" 같은 실제 서버 오류가 `ErrActorPanicked`로 오인되어 원인 분석이 불가능해진다.

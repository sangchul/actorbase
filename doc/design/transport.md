...
┌─────────┐
│  abctl  │ ─── RequestSplit / RequestMigrate / RequestMerge
└─────────┘
```
...
**PMClient** (SDK/abctl → PM): `WatchRouting`, `RequestSplit`, `RequestMigrate`, `RequestMerge`, `ListMembers`. WatchRouting 스트림이 끊기면 내부에서 자동 재연결 후 재구독한다.

**PSControlClient** (PM → PS): `ExecuteSplit(ctx, actorType, partitionID, splitKey, keyRangeStart, keyRangeEnd, newPartitionID) (string, error)` — 반환값은 PS가 실제 사용한 split key. `ExecuteMigrateOut`, `PreparePartition`, `ExecuteMerge(ctx, actorType, lowerPartitionID, upperPartitionID)`, `GetStats`.

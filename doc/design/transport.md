...
**PSClient** (SDK → PS): `Send(ctx, actorType, partitionID, req, respPtr)` — Codec으로 직렬화/역직렬화. gRPC status는 provider error로 변환하여 반환.

**PMClient** (SDK/abctl → PM): `WatchRouting`, `RequestSplit`, `RequestMigrate`, `ListMembers`. WatchRouting 스트림이 끊기면 내부에서 자동 재연결 후 재구독한다.

**PSControlClient** (PM → PS): `ExecuteSplit(ctx, actorType, partitionID, splitKey, keyRangeStart, keyRangeEnd, newPartitionID) (string, error)` — 반환값은 PS가 실제 사용한 split key. `ExecuteMigrateOut`, `PreparePartition`, `GetStats`.

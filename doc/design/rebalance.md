...
파티션 Split·Migration·Merge 조율. PM이 사용한다.
...
[PM: Splitter]                    [Source PS]
      ...
      │                                       │ 5. Actor.Export(결정된 key) → upperHalf
...
> `PSClientFactory`는 `transport.ConnPool`의 추상화 레이어로, 단위 테스트에서 mock으로 교체할 수 있다.
...
```go
func NewMigrator(routingStore cluster.RoutingTableStore, nodeCatalog cluster.NodeCatalog, psFactory transport.PSClientFactory) *Migrator
...
// PM 재시작 후 Draining 상태 파티션 복구용 (ExecuteMigrateOut 건너뜀)
func (m *Migrator) ResumeMigrate(ctx context.Context, partitionID, targetNodeID string) error
```
...
인접한 두 파티션을 하나로 합치는 조율. lower 파티션이 upper 파티션의 상태를 흡수한다.

### 전제 조건

- 두 파티션이 같은 actor type
- 인접 KeyRange (`lower.End == upper.Start`)
- 같은 노드에 위치 (다른 노드이면 balancerRunner가 먼저 upper를 lower 노드로 migrate)
- 두 파티션 모두 Active 상태
...
[PM: Merger]                        [PS]
      │
      │ 1. 검증 (actor type, 인접 KeyRange, 같은 노드, 모두 Active)
      │
      │ 2. 두 파티션 → Draining (etcd 저장)
      │    (SDK: ErrPartitionBusy 수신 → 재시도 대기)
      │
      │── ExecuteMerge ──────────────────▶│
      │   (actorType, lowerPartitionID,   │ 3. upper actor Export("") → upperData
      │    upperPartitionID)              │ 4. upper actor 삭제 + checkpoint/WAL 삭제
      │                                   │ 5. lower actor Import(upperData)
      │                                   │ 6. lower actor checkpoint
      │◀── success ───────────────────────│
      │
      │ 7. 라우팅 테이블 갱신:
      │    lower.KeyRange.End = upper.KeyRange.End
      │    upper 파티션 삭제
      │ 8. etcd 저장
```
...
// PM 재시작 후 Draining 상태 merge 복구용 (ExecuteMerge를 조건부로 건너뜀)
func (m *Merger) ResumeMerge(ctx context.Context, actorType, lowerPartitionID, upperPartitionID string) error
```
...
| 실패 시점 | 복구 방법 |
|---|---|
| ExecuteMerge 전 (Draining 저장 후) | RoutingTable을 Active로 되돌린다 |
| ExecuteMerge 실패 | RoutingTable을 Active로 되돌린다 |
| ExecuteMerge 성공 후, 라우팅 저장 실패 | etcd 저장 재시도 |
...
| 항목 | 결정 | 근거 |
|---|---|---|
| Actor 상태 분리 | `Actor.Export(splitKey)` 메서드 | Actor가 자신의 데이터 구조를 직접 분리. full snapshot 복사 후 필터링보다 정확. |
...
- **Merge 중 다운타임**: 두 파티션이 Draining 상태인 동안 SDK는 ErrPartitionBusy를 수신하고 재시도한다.
- **Merge 전제: 같은 노드**: 다른 노드의 파티션을 merge하려면 먼저 migrate가 필요하다. balancerRunner가 자동으로 처리한다.

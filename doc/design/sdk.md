...
| 필드 | 설명 |
|---|---|
| PMAddr | PM gRPC 주소. EtcdEndpoints가 없을 때 필수. |
| EtcdEndpoints | HA 모드. 설정 시 etcd에서 PM 리더 주소를 자동 조회. PM 장애 시 새 리더로 자동 재연결. PMAddr와 둘 중 하나만 설정하면 된다. |
...
| 모드 | 설정 | 동작 |
|---|---|---|
| 단일 PM | `PMAddr: "localhost:8000"` | 고정 주소로 연결. PM 장애 시 수동 조치 필요. |
| HA (etcd 발견) | `EtcdEndpoints: []string{"localhost:2379"}` | 시작·재연결 시 etcd에서 리더 PM 주소 자동 조회. 새 PM 투입 시 재시작 불필요. |
...
Start(ctx):
  ...
  3. go consumeRouting(ctx, ch)  // 이후 갱신 + PM 재연결 + ctx 취소 시 connPool.Close
  4. return nil  // 첫 RT 수신 후 즉시 반환. Send 바로 사용 가능.
```

**consumeRouting (HA 모드 재연결 포함):**

```
consumeRouting(ctx, ch):
  loop:
    for rt := range ch:
      routing.Store(rt)       // 정상 갱신
    // 채널 닫힘 = PM 연결 끊김
    if !haMode || ctx.Done: return
    // HA 모드: etcd에서 새 PM 리더 발견
    newAddr = WaitForLeader(etcdCli, 30s timeout)
    pmClient = NewPMClient(grpc.NewClient(newAddr))
    ch = pmClient.WatchRouting(ctx, clientID)
    // 재연결 후 루프 재시작
```
...
| SDK etcd 사용 (선택) | EtcdEndpoints 설정 시 시작/재연결 때 leader 조회 (point Get, persistent watch 없음) | PM 장애 시 자동 재연결. 새 PM 투입 시 SDK 재시작 불필요. |
| SDK 단일 PM 모드 | PMAddr 고정 | etcd 없이도 사용 가능. 단순한 배포에 적합. |
...
- **PM 연결 단절 시 (단일 PM 모드)**: 재연결 중에도 기존 캐시된 라우팅으로 Send는 계속 동작하나, 라우팅 변경은 반영되지 않는다. HA 모드(`EtcdEndpoints`)에서는 etcd를 통해 새 PM 리더를 발견하고 자동 재연결한다.

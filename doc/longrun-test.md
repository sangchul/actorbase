...
# abctl routing 출력 컬럼: $1=PARTITION-ID $2=ACTOR-TYPE $3=KEY-START $4=KEY-END $5=NODE-ID
get_routing_entries() {
...
do_split() {
  ...
  while IFS=$'\t' read -r pid pstart pend pnode; do
    ...
    if [[ $mid -gt $s && $mid -lt $e ]]; then
      ...
      "$ABCTL" -pm "$PM_ADDR" split kv "$pid" "$split_key"
      return
    fi
...
do_migrate() {
  ...
  while IFS=$'\t' read -r pid pstart pend pnode; do
    if [[ "$pnode" != "$target_node" ]]; then
      "$ABCTL" -pm "$PM_ADDR" migrate kv "$pid" "$target_node"
      return
    fi
  done <<< "$(get_routing_entries)"
}
...
### multi-actor-type 지원 버그 수정 (2026-03-08)

**변경 내용**: 플랫폼에 multi-actor-type 지원 추가.

**chaos.sh awk 컬럼 오류**:

- **증상**: `do_split`이 항상 "no splittable partition found"로 스킵.
- **원인**: `abctl routing` 출력에 ACTOR-TYPE 컬럼(`$2`)이 추가되어 KEY-START가 `$2`→`$3`, KEY-END `$3`→`$4`, NODE-ID `$4`→`$5`로 이동. awk 파싱이 `{print $1"\t"$2"\t"$3"\t"$4}` 그대로여서 ACTOR-TYPE("kv")이 KEY-START로 읽힘 → `key_to_num("kv")=0` → 중간점 계산 오류.
- **수정**: `get_routing_entries()`의 awk 출력을 `{print $1"\t"$3"\t"$4"\t"$5}`로 수정.

**WAL partial write 오류**:

- **증상**: SIGKILL 후 PS 재기동 시 `replay WAL entry LSN=N: unexpected end of JSON input` 에러로 actor 활성화 실패.
- **원인**: SIGKILL이 WAL flush 도중 발생하여 마지막 WAL 엔트리가 불완전하게 기록됨.
- **수정**: `engine/host.go` WAL replay 루프에서 마지막 엔트리 replay 실패 시 경고 로그 후 스킵(break). 중간 엔트리 실패는 여전히 에러 반환(실제 데이터 손상).

**수정 파일**:
- `test/longrun/chaos.sh`: `get_routing_entries()` awk 컬럼 수정, `do_split`/`do_migrate` abctl 호출에 `kv` actorType 인수 추가
- `internal/engine/host.go`: WAL replay 루프 마지막 엔트리 partial write 허용

---

### 3차 통과 (2026-03-08, multi-actor-type + WAL partial write 수정 후)

...
export PM_ADDR="localhost:7000"
export PS1_ADDR="localhost:7001"
export PS2_ADDR="localhost:7002"
...
```bash
./bin/pm -addr :7000 -etcd $ETCD_ADDR
```

예상 로그:

```
starting PM addr=:7000
```
...
```
starting PS node-id=ps-1 addr=localhost:7001
```
...
------------------------------------  ----------------  ----------------  ------------------------------------  -----------
<uuid>                                (start)           (end)             ps-1                                  localhost:7001
```
...
PARTITION-ID                          KEY-START         KEY-END           NODE-ID     NODE-ADDR
------------------------------------  ----------------  ----------------  ----------  ----------------
<original-uuid>                       (start)           m                 ps-1        localhost:7001
<new-uuid>                            m                 (end)             ps-1        localhost:7001
```
...
------------------------------------  ----------------  ----------------  ----------  ----------------
<original-uuid>                       (start)           m                 ps-1        localhost:7001
<new-uuid>                            m                 (end)             ps-2        localhost:7002
```

### 검증 포인트

- [ ] Migrate 중 라우팅 테이블에 `PartitionStatusDraining`이 잠시 표시된다.
- [ ] Migrate 완료 후 상위 파티션의 NODE-ADDR가 `localhost:7002`로 변경된다.
...
./bin/ps -node-id ps-1 -addr $PS1_ADDR -etcd $ETCD_ADDR -data-dir $DATA_DIR_1
...
→ `PreparePartition` 단계에서 PS-2가 checkpoint를 로드한다. checkpoint 파일이
  ...
  로컬 테스트에서는 `DATA_DIR_1`과 `DATA_DIR_2`를 동일 디렉토리로 설정하거나
  checkpoint 파일을 수동으로 복사한다.

```bash
# 로컬 테스트: 동일 checkpoint 디렉토리 공유
./bin/ps -node-id ps-2 -addr $PS2_ADDR -etcd $ETCD_ADDR \
  -data-dir $DATA_DIR_1   # PS-1과 동일 디렉토리 사용
```

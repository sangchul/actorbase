...
| Redis | 7.0+ (선택) | `brew install redis` — Redis WAL 백엔드 사용 시 필요 |
...
kv_server는 두 가지 WAL 백엔드를 지원한다.

| 백엔드 | 플래그 | 특징 |
|---|---|---|
| `fs` (기본) | `-wal-dir <path>` | 로컬 파일시스템, 공유 디렉토리 필요 |
| `redis` | `-wal-backend redis -redis-addr <addr>` | Redis Streams 기반, 네트워크 WAL |
...
```bash
./bin/kv_server -node-id ps-1 -addr $PS1_ADDR -etcd $ETCD_ADDR \
  -wal-backend redis -redis-addr $REDIS_ADDR \
  -checkpoint-dir $CHECKPOINT_DIR
```

Redis WAL의 장점: 공유 파일시스템 없이 PS간 WAL 공유 가능. 실제 장비 장애에도 데이터 보존.
...
# Redis WAL 백엔드 사용 시 (선택)
export WAL_BACKEND="redis"      # 기본값: fs
...
`test/integration/run.sh`로 전체 또는 특정 시나리오를 자동 실행할 수 있다.
...
# 전체 시나리오 실행 (Redis WAL)
WAL_BACKEND=redis REDIS_ADDR=localhost:6379 bash test/integration/run.sh

# 특정 시나리오만 실행
bash test/integration/run.sh 12            # 시나리오 12만
bash test/integration/run.sh 5 9 12        # 복수 선택

# Redis WAL + 특정 시나리오
WAL_BACKEND=redis bash test/integration/run.sh 5 12
```

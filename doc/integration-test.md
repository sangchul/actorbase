...
export DATA_DIR_1="/tmp/actorbase-ps1"   # PS-1 WAL 루트 (노드별 전용)
export DATA_DIR_2="/tmp/actorbase-ps2"   # PS-2 WAL 루트 (노드별 전용)
...
- [ ] Failover 후 하위 파티션 데이터(ps-1의 마지막 checkpoint에서 복원)가 ps-2에서 정상 조회된다.
...
./bin/kv_server -node-id ps-1 -addr $PS1_ADDR -etcd $ETCD_ADDR \
  -wal-dir $DATA_DIR_1/wal -checkpoint-dir $CHECKPOINT_DIR
./bin/kv_server -node-id ps-2 -addr $PS2_ADDR -etcd $ETCD_ADDR \
  -wal-dir $DATA_DIR_2/wal -checkpoint-dir $CHECKPOINT_DIR
```
